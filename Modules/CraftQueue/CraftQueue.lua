...
CraftSim.CRAFTQ = GUTIL:CreateRegistreeForEvents({
    "TRADE_SKILL_ITEM_CRAFTED_RESULT", "COMMODITY_PURCHASE_SUCCEEDED", })
...
function CraftSim.CRAFTQ:COMMODITY_PURCHASE_SUCCEEDED()
    ...
    if CraftSim.CRAFTQ.purchasedItem then
        GUTIL:ContinueOnAllItemsLoaded({ CraftSim.CRAFTQ.purchasedItem.item, },
            function()
                ...
                local success
                local result
                local shoppingListName = CraftSim.CONST
                  .AUCTIONATOR_SHOPPING_LIST_QUEUE_NAME
                success, result = pcall(Auctionator.API.v1.GetShoppingListItems,
                    addonName,
                    shoppingListName)
                if not success then
                    ...
                    shoppingListName = CraftSim.CONST
                      .AUCTIONATOR_SHOPPING_LIST_QUEUE_NAME ..
                      " " .. CraftSim.UTIL:GetPlayerCrafterUID()
                    success, result = pcall(
                        Auctionator.API.v1.GetShoppingListItems, addonName,
                        shoppingListName)
                ...
                local itemQualityID = GUTIL:GetQualityIDFromLink(purchasedItem
                    .item:GetItemLink())
                local searchTerms = {
                ...
                local oldTerms = Auctionator.API.v1.ConvertFromSearchString(
                    addonName, oldSearchString)
                ...
                if newQuantity > 0 then
                    ...
                    local newSearchString = Auctionator.API.v1
                      .ConvertToSearchString(addonName, searchTerms)
                    -- adapt
                    Auctionator.API.v1.AlterShoppingListItem(addonName,
                        shoppingListName,
                        oldSearchString, newSearchString)
                else
                    -- remove
                    Auctionator.API.v1.DeleteShoppingListItem(addonName,
                        shoppingListName,
                        oldSearchString)
                end
...
function CraftSim.CRAFTQ:InitializeCraftQueue()
    ...
    hooksecurefunc(C_AuctionHouse, "ConfirmCommoditiesPurchase",
...
function CraftSim.CRAFTQ:AddRecipe(recipeData, amount)
    ...
    CraftSim.CRAFTQ.craftQueue = CraftSim.CRAFTQ.craftQueue or
      CraftSim.CraftQueue()
...
function CraftSim.CRAFTQ.ImportRecipeScanFilter(recipeData) -- . accessor instead of : is on purpose here
    ...
    local restockOptions = CraftSim.CRAFTQ:GetRestockOptionsForRecipe(recipeData
        .recipeID)
    ...
    if not restockOptions.enabled then
        ...
        local saleRateReached = CraftSim.CRAFTQ:CheckSaleRateThresholdForRecipe(
            recipeData, nil,
    ...
    if recipeData.relativeProfitCached then
        profitMarginReached = recipeData.relativeProfitCached >=
          (restockOptions.profitMarginThreshold)
    end

    local saleRateReached = CraftSim.CRAFTQ:CheckSaleRateThresholdForRecipe(
        recipeData, restockOptions
...
function CraftSim.CRAFTQ:ImportRecipeScan()
    CraftSim.CRAFTQ.craftQueue = CraftSim.CRAFTQ.craftQueue or
      CraftSim.CraftQueue()
    local recipeScanTabContent = CraftSim.RECIPE_SCAN.frame.content
      .recipeScanTab
      .content --[[@as CraftSim.RECIPE_SCAN.RECIPE_SCAN_TAB.CONTENT]]
    local professionList = recipeScanTabContent.professionList
    local selectedRow = professionList
      .selectedRow --[[@as CraftSim.RECIPE_SCAN.PROFESSION_LIST.ROW]]
    if not selectedRow then return end -- nil check .. who knows..
    if not CraftSimOptions.recipeScanImportAllProfessions then
        ---@type CraftSim.RecipeData[]
        local filteredRecipes = GUTIL:Filter(selectedRow.currentResults,
            CraftSim.CRAFTQ.ImportRecipeScanFilter)
        for _, recipeData in pairs(filteredRecipes) do
            local restockOptions = CraftSim.CRAFTQ:GetRestockOptionsForRecipe(
                recipeData.recipeID)
            local restockAmount = tonumber(CraftSimOptions
                .craftQueueGeneralRestockRestockAmount)
            if restockOptions.enabled then
                ...
                for qualityID, use in pairs(restockOptions.restockPerQuality) do
                    if use then
                        local item = recipeData.resultData.itemsByQuality
                          [qualityID]
                        if item then
                            local itemCount = CraftSim.CRAFTQ
                              :GetItemCountFromCraftQueueCache(item:GetItemID(),
                                  true,
                                  false, true,
        ...
        local queueTab = CraftSim.CRAFTQ.frame.content
          .queueTab --[[@as CraftSim.CraftQueue.QueueTab]]
        ...
        GUTIL:FrameDistributedIteration(professionList.activeRows,
            ...
            function(_, row, counter)
                --- update button
                local checkBoxColumn = row.columns
                  [1] --[[@as CraftSim.RECIPE_SCAN.PROFESSION_LIST.CHECKBOX_COLUMN]]
                ...
                importButton:SetText(
                    "Importing: " ..
                    tostring(counter) .. "/" .. tostring(numImportProfessions),
                    15,
                    true)
                importButton:SetEnabled(false)

                ---@type CraftSim.RecipeData[]
                local filteredRecipes = GUTIL:Filter(row.currentResults,
                    CraftSim.CRAFTQ.ImportRecipeScanFilter)
                for _, recipeData in pairs(filteredRecipes) do
                    local restockOptions = CraftSim.CRAFTQ
                      :GetRestockOptionsForRecipe(recipeData.recipeID)
                    local restockAmount = tonumber(CraftSimOptions
                        .craftQueueGeneralRestockRestockAmount)
                    if restockOptions.enabled then
                        restockAmount = restockOptions.restockAmount
                        ...
                        for qualityID, use in pairs(restockOptions.restockPerQuality) do
                            if use then
                                local item = recipeData.resultData
                                  .itemsByQuality[qualityID]
                                if item then
                                    local itemCount = CraftSim.CRAFTQ
                                      :GetItemCountFromCraftQueueCache(
                                          item:GetItemID(),
                                          true, false,
                    ...
                    if restockAmount > 0 then
                        CraftSim.CRAFTQ.craftQueue:AddRecipe(recipeData,
                            restockAmount)
                    end
                end
...
function CraftSim.CRAFTQ:DeleteAllCraftSimShoppingLists()
    ...
    CraftSim.CRAFTQ:DeleteAuctionatorShoppingList(CraftSim.CONST
        .AUCTIONATOR_SHOPPING_LIST_QUEUE_NAME)
...
function CraftSim.CRAFTQ.CreateAuctionatorShoppingListPerCharacter()
    print("CraftSim.CRAFTQ:CreateAuctionatorShoppingListPerCharacter", false,
        true)
    ...
    for _, craftQueueItem in pairs(CraftSim.CRAFTQ.craftQueue.craftQueueItems) do
        local requiredReagents = craftQueueItem.recipeData.reagentData
          .requiredReagents
        ...
        for _, reagent in pairs(requiredReagents) do
            if reagent.hasQuality then
                for qualityID, reagentItem in pairs(reagent.items) do
                    reagentMapPerCharacter[crafterUID][reagentItem.item:GetItemID()] =
                      reagentMapPerCharacter
                      [crafterUID][reagentItem.item:GetItemID()] or {
                          itemName = reagentItem.item:GetItemName(),
                          qualityID = nil,
                          quantity = 0,
                      }
                    reagentMapPerCharacter[crafterUID][reagentItem.item:GetItemID()].quantity =
                      reagentMapPerCharacter
                      [crafterUID][reagentItem.item:GetItemID()]
                      .quantity + (reagentItem.quantity * craftQueueItem.amount)
                    reagentMapPerCharacter[crafterUID][reagentItem.item:GetItemID()].qualityID =
                      qualityID
                end
            else
                ...
                print("reagentMap Build: " ..
                    tostring(reagentItem.item:GetItemLink()))
                print("quantity: " ..
                    tostring(reagentMapPerCharacter[crafterUID]
                        [reagentItem.item:GetItemID()].quantity))
            end
        end
        local activeReagents = craftQueueItem.recipeData.reagentData
          :GetActiveOptionalReagents()
        for _, optionalReagent in pairs(activeReagents) do
            if not GUTIL:isItemSoulbound(optionalReagent.item:GetItemID()) then
                reagentMapPerCharacter[crafterUID][optionalReagent.item:GetItemID()] =
                  reagentMapPerCharacter
                ...
                reagentMapPerCharacter[crafterUID][optionalReagent.item:GetItemID()].quantity =
                  reagentMapPerCharacter
    ...
    for crafterUID, reagentMap in pairs(reagentMapPerCharacter) do
        ...
        local searchStrings = GUTIL:Map(reagentMap, function(info, itemID)
            ...
            local itemCount = CraftSim.CACHE.ITEM_COUNT:Get(itemID, true, false,
                true, crafterUID)
            local searchTerm = {
...
function CraftSim.CRAFTQ.CreateAuctionatorShoppingListAll()
    ...
    for _, craftQueueItem in pairs(CraftSim.CRAFTQ.craftQueue.craftQueueItems) do
        ...
        for _, reagent in pairs(requiredReagents) do
            if reagent.hasQuality then
                for qualityID, reagentItem in pairs(reagent.items) do
                    reagentMap[reagentItem.item:GetItemID()] = reagentMap
                      [reagentItem.item:GetItemID()] or {
                          itemName = reagentItem.item:GetItemName(),
                          qualityID = nil,
                          quantity = 0,
                      }
                    reagentMap[reagentItem.item:GetItemID()].quantity =
                      reagentMap[reagentItem.item:GetItemID()]
                      .quantity + (reagentItem.quantity * craftQueueItem.amount)
                    reagentMap[reagentItem.item:GetItemID()].qualityID =
                      qualityID
                end
            else
                local reagentItem = reagent.items[1]
                ...
                reagentMap[reagentItem.item:GetItemID()].quantity = reagentMap
                  [reagentItem.item:GetItemID()].quantity +
                  (reagentItem.quantity * craftQueueItem.amount)
                print("reagentMap Build: " ..
                    tostring(reagentItem.item:GetItemLink()))
                print("quantity: " ..
                    tostring(reagentMap[reagentItem.item:GetItemID()].quantity))
            end
        end
        local activeReagents = craftQueueItem.recipeData.reagentData
          :GetActiveOptionalReagents()
        for _, optionalReagent in pairs(activeReagents) do
            if not GUTIL:isItemSoulbound(optionalReagent.item:GetItemID()) then
                reagentMap[optionalReagent.item:GetItemID()] = reagentMap
                  [optionalReagent.item:GetItemID()] or {
                      itemName = optionalReagent.item:GetItemName(),
                      qualityID = optionalReagent.qualityID,
                      quantity = 0,
                  }
                reagentMap[optionalReagent.item:GetItemID()].quantity =
                  reagentMap[optionalReagent.item:GetItemID()]
                  .quantity + craftQueueItem.amount
            end
        end
    end

    local crafterUIDs = GUTIL:Map(CraftSim.CRAFTQ.craftQueue.craftQueueItems,
        function(cqi)
    ...
    local searchStrings = GUTIL:Map(reagentMap, function(info, itemID)
        ...
        local totalItemCount = GUTIL:Fold(crafterUIDs,
            function(itemCount, crafterUID)
                local itemCountForCrafter = CraftSim.CRAFTQ
                  :GetItemCountFromCraftQueueCache(itemID, true, false, true,
                      crafterUID)
                _G.print("D", itemCountForCrafter)
        ...
        local searchString = Auctionator.API.v1.ConvertToSearchString(addonName,
            searchTerm)
        return searchString
    end)
    Auctionator.API.v1.CreateShoppingList(addonName,
        CraftSim.CONST.AUCTIONATOR_SHOPPING_LIST_QUEUE_NAME, searchStrings)
...
function CraftSim.CRAFTQ:TRADE_SKILL_ITEM_CRAFTED_RESULT()
    ...
    if CraftSim.CRAFTQ.currentlyCraftedRecipeData then
        ...
        CraftSim.CRAFTQ.craftQueue:SetAmount(
            CraftSim.CRAFTQ.currentlyCraftedRecipeData, -1, true)
        CraftSim.CRAFTQ.FRAMES:UpdateDisplay()
    end
end

--- only for craft queue display update's flash cache
function CraftSim.CRAFTQ:GetItemCountFromCraftQueueCache(itemID, bank, uses,
    reagentbank, crafterUID)
    local itemCount = (CraftSim.CRAFTQ.itemCountCache and CraftSim.CRAFTQ.itemCountCache[itemID]) or
      nil
    if not itemCount then
        itemCount = CraftSim.CACHE.ITEM_COUNT:Get(itemID, bank, uses, reagentbank,
            crafterUID)
    end
    return itemCount
end
...
function CraftSim.CRAFTQ:GetAuctionatorShoppingListItemString(itemName, qualityID,
    quantity)
    return '"' ..
      itemName ..
      '"' ..
      ";;0;0;0;0;0;0;0;0;;" .. (qualityID or "#") .. ";;" .. (quantity or "")
end
...
function CraftSim.CRAFTQ:ParseAuctionatorShoppingListItemString(itemString)
    local name, qualityID, quantity = itemString:match(
        '"([^"]+)";;0;0;0;0;0;0;0;0;;(%S+);;(%d+)$')
    if qualityID == "#" then
        qualityID = nil
    else
...
function CraftSim.CRAFTQ:GetRestockOptionsForRecipe(recipeID)
    CraftSimOptions.craftQueueRestockPerRecipeOptions[recipeID] = CraftSimOptions
      .craftQueueRestockPerRecipeOptions
      [recipeID] or {}
    return {
        enabled = CraftSimOptions.craftQueueRestockPerRecipeOptions[recipeID]
          .enabled or false,
        profitMarginThreshold = CraftSimOptions
          .craftQueueRestockPerRecipeOptions[recipeID].profitMarginThreshold or 0,
        restockAmount = CraftSimOptions.craftQueueRestockPerRecipeOptions
          [recipeID].restockAmount or 1,
        restockPerQuality = CraftSimOptions.craftQueueRestockPerRecipeOptions
          [recipeID].restockPerQuality or {},
        saleRateThreshold = CraftSimOptions.craftQueueRestockPerRecipeOptions
          [recipeID].saleRateThreshold or 0,
        saleRatePerQuality = CraftSimOptions.craftQueueRestockPerRecipeOptions
          [recipeID].saleRatePerQuality or {},
    }
end
...
---@private
function CraftSim.CRAFTQ:CheckSaleRateThresholdForRecipe(recipeData,
    usedQualitiesTable, saleRateThreshold)
    ...
    for qualityID, checkQuality in pairs(usedQualitiesTable or {}) do
        ...
        if item and checkQuality then
            print("check sale rate for q" ..
                qualityID .. ": " .. tostring(checkQuality))
...
function CraftSim.CRAFTQ:OnRecipeEditSave()
    ...
    local editRecipeFrame = GGUI:GetFrame(CraftSim.MAIN.FRAMES,
